<?php
// models/SeekerModel.php — All database logic for Job Seeker role
// NO business logic in views. All queries use mysqli prepared statements.
...
class SeekerModel {
    ...
    public function upsertSeekerProfile(
        ...
        $stmt->bind_param('isssisdss', $userId, $headline, $summary, $skills, $yearsExp,
    ...
    public function updateResumePath(int $userId, string $resumePath): bool {
        $db = getDB();
        $stmt = $db->prepare(
            "UPDATE seeker_profiles SET resume_path = ? WHERE user_id = ?"
        );
        $stmt->bind_param('si', $resumePath, $userId);
        return $stmt->execute();
    }
     ...
     * Returns array of job rows joined with employer/recruiter info.
     * Used by both full-page load and AJAX endpoint.
     */
    ...
    // ─── NOTIFICATIONS (matched alerts) ──────────────────────────────────────

    /**
     * Find jobs posted after $since that match any of seeker's alerts.
     * Called when seeker logs in or via cron.
     */
