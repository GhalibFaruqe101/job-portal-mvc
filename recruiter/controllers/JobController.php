...
function validateJobForm() {
    $errors = [];
    if (empty(trim($_POST['title'] ?? '')))       $errors[] = "Job title is required.";
    if (empty(trim($_POST['description'] ?? '')))  $errors[] = "Job description is required.";
    if (empty($_POST['client_id'] ?? ''))           $errors[] = "Please select a client.";
    if (empty($_POST['category_id'] ?? ''))         $errors[] = "Please select a category.";
    if (empty($_POST['job_type'] ?? ''))             $errors[] = "Job type is required.";
    if (empty($_POST['experience_level'] ?? ''))     $errors[] = "Experience level is required.";

    $salary_min = floatval($_POST['salary_min'] ?? 0);
    $salary_max = floatval($_POST['salary_max'] ?? 0);
    if ($salary_min < 0) $errors[] = "Minimum salary cannot be negative.";
    if ($salary_max < 0) $errors[] = "Maximum salary cannot be negative.";
